...
class TestTransAndRewards(unittest.TestCase):
    ...
    def test_function(self):
        ...
        self.assertEqual(opt.get_transition_probs(a_state, a_state), 0.0)
        self.assertEqual(opt.get_transition_probs(a_state, b_state), 0.6)
        self.assertEqual(opt.get_transition_probs(a_state, c_state), 0.4)
        self.assertEqual(opt.get_transition_probs(b_state, a_state), 0.3)
        self.assertEqual(opt.get_transition_probs(b_state, b_state), 0.0)
        self.assertEqual(opt.get_transition_probs(b_state, c_state), 0.7)
        self.assertEqual(opt.get_transition_probs(c_state, a_state), 0.0)
        self.assertEqual(opt.get_transition_probs(c_state, b_state), 0.0)
        self.assertEqual(opt.get_transition_probs(c_state, c_state), 0.0)
